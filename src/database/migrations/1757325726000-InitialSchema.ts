...
/**
 * Initial schema for the Devstree Receipt System.
 *
 * Notable design choices enforced here (see docs/security.md):
 *  - `projects.encrypted_amount` / `payments.encrypted_amount` are the ONLY
 *    representation of a monetary value — there is no plaintext `amount` column.
 *  - `document_sequences` backs gap-free, lock-safe receipt/client/project numbering.
 *  - Payments are never physically deleted (status VOIDED instead) to preserve
 *    the audit trail and receipt history.
 */
export class InitialSchema1757325726000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "payment_method_enum" AS ENUM ('BANK_TRANSFER','UPI','CHEQUE','CASH','CREDIT_CARD','OTHER')`,
    );
    await queryRunner.query(
      `CREATE TYPE "payment_status_enum" AS ENUM ('VALID','VOIDED','CANCELLED')`,
    );
    ...
    `);
    await queryRunner.query(
      `CREATE INDEX "idx_password_histories_user_type" ON "password_histories" ("user_id", "password_type")`,
    );
    ...
    // ── Projects (financial amount stored only as ciphertext) ───
    await queryRunner.query(`
    ...
    // ── Payments (financial amount stored only as ciphertext) ───
    await queryRunner.query(`
    ...
    await queryRunner.query(
      `CREATE UNIQUE INDEX "idx_receipts_payment_id" ON "receipts" ("payment_id")`,
    );
    await queryRunner.query(
    ...
    // ── Financial unlock sessions & attempts ─────────────────────
    await queryRunner.query(`
    ...
    `);
    await queryRunner.query(
      `CREATE INDEX "idx_financial_sessions_user" ON "financial_unlock_sessions" ("user_id")`,
    );
    await queryRunner.query(
      `CREATE UNIQUE INDEX "idx_financial_sessions_token_hash" ON "financial_unlock_sessions" ("session_token_hash")`,
    );
    await queryRunner.query(
      `CREATE INDEX "idx_financial_sessions_expires" ON "financial_unlock_sessions" ("expires_at")`,
    );
    ...
    `);
    await queryRunner.query(
      `CREATE INDEX "idx_financial_attempts_user_created" ON "financial_unlock_attempts" ("user_id", "created_at")`,
    );
    ...
    // ── Document sequences (receipt/client/project numbering) ───
    await queryRunner.query(`
