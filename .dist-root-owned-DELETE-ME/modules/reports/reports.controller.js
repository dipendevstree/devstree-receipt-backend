...
let ReportsController = class ReportsController {
    ...
    async clientCollection(format, res) {
        const rows = await this.reportsService.clientCollection(true);
        return this.respond(res, format, 'client-collection', rows, [
            { key: 'clientName', header: 'Client' },
            { key: 'projectCount', header: 'Projects' },
            { key: 'totalProjectValue', header: 'Project Value' },
            { key: 'totalReceived', header: 'Received' },
            { key: 'totalDue', header: 'Due' },
        ]);
    }
    async projectCollection(format, res) {
        const rows = await this.reportsService.projectCollection(true);
        return this.respond(res, format, 'project-collection', rows, [
            { key: 'clientName', header: 'Client' },
    ...
    async monthlyCollection(months, format, res) {
        const parsed = Math.min(24, Math.max(1, Number(months) || 12));
        const rows = await this.reportsService.monthlyCollection(parsed, true);
        return this.respond(res, format, 'monthly-collection', rows, [
...
__decorate([
    ...
    (0, swagger_1.ApiOperation)({ summary: 'Projects with an outstanding due amount' }),
...
__decorate([
    ...
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [payment_dto_1.QueryPaymentDto, Object, Object]),
    __metadata("design:returntype", Promise)
