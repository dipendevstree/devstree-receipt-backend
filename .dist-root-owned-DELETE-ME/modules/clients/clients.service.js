...
let ClientsService = class ClientsService {
    ...
    async findAll(query, unlocked) {
        ...
        const items = clients.map((client) => {
            const clientTotals = totals.get(client.id);
            return this.toResponse(client, unlocked, {
                projectCount: clientTotals?.projectCount ?? 0,
                totalProjectValue: clientTotals
                    ? (0, mask_util_1.maskAmount)(clientTotals.totalProjectValue, unlocked)
                    : null,
                totalReceived: clientTotals ? (0, mask_util_1.maskAmount)(clientTotals.totalReceived, unlocked) : null,
                totalDue: clientTotals ? (0, mask_util_1.maskAmount)(clientTotals.totalDue, unlocked) : null,
            });
        });
    ...
    async findOne(id, unlocked) {
        ...
        const totals = await this.financials.totalsForClient(id);
        return this.toResponse(client, unlocked, {
    ...
    async create(dto, actor, context) {
        ...
        const client = await this.dataSource.transaction(async (manager) => {
            ...
            const entity = manager.getRepository(client_entity_1.Client).create({ ...dto, clientCode });
        ...
        return this.toResponse(client, false, {
            projectCount: 0,
    ...
    async lookup(search) {
        const builder = this.clients
            ...
            .select(['client.id', 'client.name', 'client.company_name', 'client.client_code'])
        ...
        if (search) {
            builder.andWhere(new typeorm_2.Brackets((qb) => qb
                .where('client.name ILIKE :term', { term: `%${search}%` })
                .orWhere('client.company_name ILIKE :term', { term: `%${search}%` })));
        }
        const clients = await builder.getMany();
        return clients.map((client) => ({
            id: client.id,
            name: client.companyName ? `${client.name} (${client.companyName})` : client.name,
            clientCode: client.clientCode,
        }));
    }
    async getOrFail(id) {
        const client = await this.clients.findOne({ where: { id, deletedAt: (0, typeorm_2.IsNull)() } });
        if (!client) {
    ...
    toResponse(client, unlocked, financials) {
        return {
            id: client.id,
            clientCode: client.clientCode,
            name: client.name,
            ...
            notes: client.notes,
            status: client.status,
            projectCount: financials.projectCount,
