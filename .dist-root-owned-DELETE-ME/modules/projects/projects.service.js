...
let ProjectsService = class ProjectsService {
    ...
    async create(dto, actor, context) {
        ...
        const amount = money_util_1.Money.fromDecimalString(dto.projectAmount);
        const encrypted = this.encryption.encryptAmount(amount, financial_encryption_service_1.EncryptionContext.PROJECT_AMOUNT);
        ...
        await this.auditLog.record({
            ...
            newValue: { projectName: dto.projectName, clientId: dto.clientId, status: project.status },
            actor,
            context,
        });
        project.client = client;
        return this.toResponse(project, null, false);
    }
    async update(id, dto, actor, context, unlocked) {
        ...
        if (dto.projectAmount !== undefined) {
            const amount = money_util_1.Money.fromDecimalString(dto.projectAmount);
            Object.assign(project, this.encryption.encryptAmount(amount, financial_encryption_service_1.EncryptionContext.PROJECT_AMOUNT));
        }
        ...
        await this.auditLog.record({
            ...
            oldValue: before,
            newValue: { projectName: project.projectName, status: project.status },
            actor,
            context,
        });
