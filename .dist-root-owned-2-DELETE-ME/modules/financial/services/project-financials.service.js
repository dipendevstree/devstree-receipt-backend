...
let ProjectFinancialsService = class ProjectFinancialsService {
    ...
    buildTotals(project, payments, precision) {
        ...
        return {
            ...
            totalReceived,
            dueAmount: projectAmount === null ? null : projectAmount.subtract(totalReceived).clampToZero(),
    ...
    resolvePaymentStatus(projectAmount, totalReceived) {
        if (projectAmount === null)
            return project_status_enum_1.ProjectPaymentStatus.VARIABLE;
...
function emptyClientTotals(clientId, precision) {
    return {
        clientId,
        projectCount: 0,
        fixedProjectCount: 0,
        ...
        fixedReceived: money_util_1.Money.zero(precision),
        variableReceived: money_util_1.Money.zero(precision),
        totalDue: money_util_1.Money.zero(precision),
    };
}
function accumulate(bucket, totals) {
